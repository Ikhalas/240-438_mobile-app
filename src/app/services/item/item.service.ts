...
import { map, filter, take, skip, withLatestFrom, switchAll } from 'rxjs/operators';
...
export class ItemService {
  ...
  get(): Observable<Items> {
    ...
    const selector = ({ refresh = false, offset, limit }, ids) => combineLatest(...(ids.slice(offset, offset + limit).map(id => this.db.object<Item>('/v0/item/' + id).
      valueChanges()))
    ).pipe(
      map(items => ({
        refresh,
